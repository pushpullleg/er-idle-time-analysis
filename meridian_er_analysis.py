...
df = pd.read_excel('Sample_CleanedData.xlsx')

# Convert timestamp columns to datetime
time_cols = ['Arrival Time', 'Registration', 'Registration', 'Triage Start', 
...
df['Wait_Registration'] = (df['Registration'] - df['Arrival Time']).dt.total_seconds() / 60
df['Wait_Triage'] = (df['Triage Start'] - df['Registration']).dt.total_seconds() / 60
